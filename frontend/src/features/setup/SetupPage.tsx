...
function DockerOption({ dockerInstalled, imageReady, onReady }: {
  ...
  return (
    <div className="border border-[var(--hud-border)] rounded-lg p-5 bg-[var(--bg-panel)] space-y-4">
      ...
      {logs.length > 0 && (
        <div className="bg-black/60 rounded p-3 h-48 overflow-y-auto font-mono text-xs text-[var(--text-primary)] space-y-0.5">
          {logs.map((line, i) => (
...
function LocalOption({ os, onReady }: {
    ...
    <div className="border border-[var(--hud-border)] rounded-lg p-5 bg-[var(--bg-panel)] space-y-4">
      ...
      {nmapInfo?.installed ? (
        ...
        <button
          onClick={handleCheck}
          disabled={checking}
          className="w-full py-3 rounded border border-[var(--radar-green)] bg-[var(--bg-panel)] text-[var(--radar-green)] font-mono text-sm uppercase tracking-wider hover:bg-[var(--radar-green-dim)] transition-colors disabled:opacity-30"
        >
          {checking ? 'Checking...' : 'Check for Nmap'}
        </button>
      )}
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-dark)] flex items-center justify-center">
      <div className="text-center space-y-4">
        <div className="w-4 h-4 rounded-full bg-[var(--radar-green)] animate-pulse mx-auto" />
        <p className="text-[var(--text-dim)] font-mono text-sm animate-pulse">
          Checking system...
        </p>
      </div>
    </div>
